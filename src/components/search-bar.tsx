...
export default function SearchBar({ handleSearchbar, toFavorite }: { handleSearchbar: (e: string) => void; toFavorite: () => void; }) {
    ...
    return (
        <div className="w-full fixed top-0 left-0 lg:relative p-8 bg-[rgba(0,0,0,.75)] lg:bg-transparent h-[5rem] lg:px-0 lg:py-4 flex justify-between pt-8 lg:h-[6rem] items-center">
            {/* @ts-ignore */}
            <button id="searchbar" onClick={e => handleSearchbar("searchbar")} className="bg-[#6e707a] p-2 lg:p-2 text-white shadow text-[#e7e7eb] w-[max-content] rounded-full lg:rounded-none lg:h-[max-content] hidden lg:block">
                Search
            </button>
            <button id="searchbar" onClick={e => handleSearchbar("searchbar")} className="bg-[#6e707a] p-2 lg:p-2 text-white shadow text-[#e7e7eb] w-[max-content] rounded-full lg:rounded-none lg:h-[max-content] block lg:hidden">
            ...
            <div className="flex items-center gap-4">
                <button className="bg-[#6e707a] lg:block hidden lg:p-2 p-2 shadow rounded-full" onClick={addToFavorites}>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="h-[2rem] w-[2rem] lg:w-6 lg:h-6 text-[#fef301]">
                ...
                <button className="bg-[#6e707a] lg:p-2 p-2 shadow rounded-full" onClick={getCurrentLocation}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="h-[1.8rem] w-[2rem] lg:w-6 lg:h-6 text-[#e7e7eb]">
