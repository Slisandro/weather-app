...
export default function FavoriteList({ onBack }: { onBack: () => void; }) {
    const { favorites, current, setCurrent, addFavorite, deleteFavorite } = useStore(s => s);
    ...
    return (
        <div className="w-full px-2 mt-8 md:hidden block">
            ...
            <ul className="w-full p-0 flex flex-col gap-8">
                {favorites.length ? favorites.map((item) => (
                    ...
                    >
                        ...
                        <div className="flex gap-4 mt-2 lg:mt-2 lg:gap-2">
                            <button onClick={() => handleFavorite(item)} className="bg-[#6e707a] p-2 lg:p-2 shadow rounded-full">
                                {isFavorite(favorites, current) ? (
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 lg:w-6 lg:h-6 text-[#fef301]">
                                        <path fillRule="evenodd" d="M10.788 3.21c.448-1.077 1.976-1.077 2.424 0l2.082 5.007 5.404.433c1.164.093 1.636 1.545.749 2.305l-4.117 3.527 1.257 5.273c.271 1.136-.964 2.033-1.96 1.425L12 18.354 7.373 21.18c-.996.608-2.231-.29-1.96-1.425l1.257-5.273-4.117-3.527c-.887-.76-.415-2.212.749-2.305l5.404-.433 2.082-5.006z" clipRule="evenodd" />
                                    </svg>
                                ) : (
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 lg:w-6 lg:h-6 text-[#e7e7eb]">
                            ...
                            <button onClick={() => handleClick(item)} className="bg-[#6e707a] p-2 lg:p-2 flex items-center justify-center shadow rounded-full">
                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6 lg:w-6 lg:h-6 text-[#e7e7eb]">
                    ...
                    <li className="flex items-center text-3xl lg:text-medium justify-between h-[max-content] mt-8">
